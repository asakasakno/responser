...
import { PLAN_LIMITS, type Product, type PlanType } from '@/types';
import { Button } from '@/components/ui/button';
import { Textarea } from '@/components/ui/textarea';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { useToast } from '@/hooks/use-toast';
import { Copy, Image, Loader2, Upload, ArrowUp, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export default function Generate() {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('generate-response', {
        body: {
          type: genType,
          text: inputText,
          product: product ? { name: product.name, category: product.category, note: product.note } : null,
        },
      });
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!limits.imageUpload) {
      toast({ title: '이미지 처리 불가', description: '이미지 일괄 처리는 Basic 이상에서 가능합니다.', variant: 'destructive' });
      return;
    }
    setImageFile(file);
    setBatchResults([]);
      ...
      <div className="p-6 md:p-8 max-w-3xl mx-auto">
        ...
        <div className="mb-4">
          <label className="text-sm font-medium text-foreground mb-1.5 block">
            {TYPE_LABELS[genType]} 내용 입력
          </label>
          <Textarea
            value={inputText}
            onChange={e => setInputText(e.target.value)}
            placeholder={
              genType === 'review' ? '고객 리뷰 내용을 입력하세요...' :
              genType === 'inquiry' ? '고객 문의 내용을 입력하세요...' :
              '클레임 내용을 입력하세요...'
            }
            rows={4}
          />
        ...
        <div className="flex gap-3 mb-6">
          ...
          <div className="relative">
            <input
              ...
              disabled={!limits.imageUpload}
            />
            <Button variant="outline" disabled={!limits.imageUpload}>
          ...
          <div className="bg-destructive/10 rounded-xl border border-destructive/20 p-4 mb-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-foreground">
                이미지에서 {extractedItems.length}개가 추출되었지만, 현재 플랜에서는 {limits.maxPerImage}개만 처리됩니다.
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                Pro 플랜으로 업그레이드하면 최대 30개까지 처리 가능합니다.
              </p>
            </div>
          </div>
        )}
              ...
              {batchResults.map((r, i) => (
                <div key={i} className="bg-card rounded-lg border border-border p-4 shadow-card">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => copyToClipboard(r)}>
