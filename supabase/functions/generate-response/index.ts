...
serve(async (req) => {
  ...
  let completeReservation: (status: "success" | "failed" | "failed_timeout" | "failed_refund", code?: string) => Promise<void> = async () => {};
  ...
  try {
    ...
    completeReservation = async (status: "success" | "failed" | "failed_timeout" | "failed_refund", code?: string) => {
      try {
    ...
    // Helper: refund energy if AI fails after spend (result must be verified)
    const refund = async (): Promise<{ ok: boolean }> => {
      const { data: claimData } = await userClient.rpc("try_claim_generate_refund", {
        _reservation_id: reservationId,
      });
      const claimed = (claimData as any)?.claimed === true;
      if (!claimed) {
        await completeReservation("failed", "ai_failed_already_refund_handled");
        return { ok: true };
      }
      ...
      try {
        ...
        await userClient.rpc("complete_generate_refund", {
          _reservation_id: reservationId,
          _refunded: refundOk,
          _refund_error: refundOk ? null : ((refundData as any)?.error ?? "refund_rpc_failed"),
        });
        if (!refundOk) {
          await adminClient.from("audit_logs").insert({
            user_id: userId,
            action: "generate_refund_failed",
            details: { reservation_id: reservationId, type, energy_cost: cost },
            severity: "error",
          });
    ...
    try {
      ...
      if (!rr.ok) {
        return jsonRes({ error: "AI 호출은 실패했고 환불 처리도 실패했습니다. 고객센터로 문의해주세요.", reservation_id: reservationId }, corsHeaders, 500);
      }
    ...
    return jsonRes({ error: "요청을 처리할 수 없습니다.", reservation_id: reservationId }, corsHeaders, 500);
  }
});
